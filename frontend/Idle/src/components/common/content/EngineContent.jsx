...
function EngineContent({ img_url, peak_output, max_torque, min_fuel, max_fuel }) {
    ...
    const formedData = { "최고 출력": { unit: "PS", value: peak_output }, "최대 토크": { unit: "kgf-m", value: max_torque }, "복합 연비": { unit: "km/L", value: [min_fuel, max_fuel] } }
    return (
        <StContainer>
            <StImage src={img_url} />
            <StDetailContaier>
                {states.map((item, idx) => (<EngineDetail key={idx} state={item} data={formedData} />))}
            </StDetailContaier>
        </StContainer>
    )
}
